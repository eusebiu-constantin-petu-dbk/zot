...
func TestDedupe(t *testing.T) {
	Convey("Dedupe", t, func(c C) {
		...
		Convey("Valid ImageStore", func() {
			...
			log := log.Logger{Logger: zerolog.New(os.Stdout)}
			metrics := monitoring.NewMetricsServer(false, log)
			il := storage.NewImageStore(dir, true, storage.DefaultGCDelay, true, true, log, metrics, lint.NewLinter(nil, log))
...
func TestNegativeCases(t *testing.T) {
	Convey("Invalid root dir", t, func(c C) {
		...
		So(storage.NewImageStore(dir, true, storage.DefaultGCDelay, true,
			true, log, metrics, lint.NewLinter(nil, log)), ShouldNotBeNil)
		if os.Geteuid() != 0 {
			So(storage.NewImageStore("/deadBEEF", true, storage.DefaultGCDelay,
				true, true, log, metrics, lint.NewLinter(nil, log)), ShouldBeNil)
		}
	})
...
func TestInjectWriteFile(t *testing.T) {
	...
	Convey("writeFile without commit", t, func() {
		...
		metrics := monitoring.NewMetricsServer(false, log)
		imgStore := storage.NewImageStore(dir, true, storage.DefaultGCDelay,
			true, false, log, metrics, lint.NewLinter(nil, log))
...
func TestGarbageCollect(t *testing.T) {
	Convey("Repo layout", t, func(c C) {
		...
		Convey("Garbage collect with short delay", func() {
			imgStore := storage.NewImageStore(dir, true, 1*time.Second, true, true, log, metrics, lint.NewLinter(nil, log))
			repoName := "gc-short"
		...
		Convey("Garbage collect with dedupe", func() {
			...
			imgStore := storage.NewImageStore(dir, true, 5*time.Second, true, true, log, metrics, lint.NewLinter(nil, log))
