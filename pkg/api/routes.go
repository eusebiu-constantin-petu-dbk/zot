	...
	if err != nil {
		if errors.Is(err, zerr.ErrRepoNotFound) || errors.Is(err, zerr.ErrManifestNotFound) {
			...
			if routeHandler.c.Config.Extensions != nil && routeHandler.c.Config.Extensions.Sync != nil {
				...
				value, loaded := routeHandler.demandedImages.LoadOrStore(image, make(chan bool))
				imageChannel, _ := value.(chan bool)

				if loaded {
					// download already started by another goroutine
					// wait to finish image download
					res, ok := <-imageChannel
					// if value received or channel closed means sync finished
					if res || !ok {
						content, digest, mediaType, err = imgStore.GetImageManifest(name, reference)
					}
				} else {
					defer routeHandler.demandedImages.Delete(fmt.Sprintf("%s:%s", name, reference))
					// start downloading image
					go downloadImage(routeHandler, name, reference)
					// wait to get result
					res := <-imageChannel
					// close channel
					close(imageChannel)
